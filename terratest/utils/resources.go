...
func (r *Resources) IngressPatchAnnotation(name, value string) {
	json := fmt.Sprintf("{\"metadata\":{\"annotations\":{\"%s\":\"%s\"}}}", name, value)
	cmd := shell.Command{
		Command: "kubectl",
		Args: []string{"--context", r.i.w.k8sOptions.ContextName, "-n", r.i.w.namespace, "patch", "ingress", r.i.w.ingress.name,
			"-p", json, "--type=merge"},
	}
	shell.RunCommandAndGetOutput(r.i.w.t, cmd)
}
