...
// WaitUntilNamespaceNotFound waits until namespace doesn't exist.
// The function is not called automatically in the kill, because we don't
// care about namespace existence in deferred Kill(). Use this function
// explicitly.
func (i *Instance) WaitUntilNamespaceNotFound() error {
	i.w.t.Logf("Wait until namespace %s not found", i.w.namespace)
	for q := 0; q < defaultRetries; q++ {
		_, err := k8s.GetNamespaceE(i.w.t, i.w.k8sOptions, i.w.namespace)
		if err != nil && strings.HasSuffix(err.Error(), "not found") {
			i.w.t.Logf("SUCCEED: Namespace %s doesn't exist", i.w.namespace)
			return nil
		}
		i.w.t.Logf("Namespace %s exist, waiting...", i.w.namespace)
		time.Sleep(defaultSeconds * time.Second)
	}
	return fmt.Errorf("FAIL: Namespace %s exists", i.w.k8gbNamespace)
}
