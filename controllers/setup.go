...
func (r *AnnoReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	serviceEndpointHandler := handler.EnqueueRequestsFromMapFunc(
		func(a client.Object) []reconcile.Request {
			var err error
			var nnv1 []types.NamespacedName
			var rr []reconcile.Request
			var nnv1beta1 []types.NamespacedName

			c := mgr.GetClient()
			if nnv1, err = converter.NewIngressV1(c).ListIngresses(a); err != nil {
				r.Log.Info().Msg("Can't fetch ingress v1 objects")
				return nil
			}
			if nnv1beta1, err = converter.NewIngressV1Beta1(c).ListIngresses(a); err != nil {
				r.Log.Info().Msg("Can't fetch ingress v1beta1 objects")
				return nil
			}
			for _, nn := range append(nnv1, nnv1beta1...) {
				rr = append(rr, reconcile.Request{NamespacedName: nn})
			}
			return rr
		})
