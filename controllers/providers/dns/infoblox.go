...
func (p *InfobloxProvider) CreateZoneDelegationForExternalDNS(rs *rs.ReconciliationState) error {
	...
	if findZone != nil {
		...
		if len(findZone.Ref) > 0 {
			...
			if !reflect.DeepEqual(findZone.DelegateTo, currentList) {
				...
				m.InfobloxIncrementZoneUpdate(rs)
			}
		}
	} else {
		log.Info().
...
func (p *InfobloxProvider) Finalize(rs *rs.ReconciliationState) error {
	objMgr, err := p.client.GetObjectManager()
	if err != nil {
		return err
	}
...
func (p *InfobloxProvider) IngressExposedIPs(rs *rs.ReconciliationState) ([]string, error) {
	return p.assistant.IngressExposedIPs(rs)
}

func (p *InfobloxProvider) SaveDNSEndpoint(rs *rs.ReconciliationState, i *externaldns.DNSEndpoint) error {
...
func (p *InfobloxProvider) saveHeartbeatTXTRecord(objMgr *ibcl.ObjectManager, rs *rs.ReconciliationState) (err error) {
	...
	if heartbeatTXTRecord == nil {
		...
		if err != nil {
			m.InfobloxIncrementHeartbeatError(rs)
			return
		}
	} else {
		log.Info().
	...
	m.InfobloxIncrementHeartbeat(rs)
	return
}
