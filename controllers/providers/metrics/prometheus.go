...
func (m *PrometheusMetrics) UpdateIngressHostsPerStatusMetric(rs *rs.ReconciliationState, serviceHealth map[string]status.HealthStatus) {
...
func (m *PrometheusMetrics) UpdateHealthyRecordsMetric(rs *rs.ReconciliationState, healthyRecords map[string][]string) {
	var hrsCount int
	for _, hrs := range healthyRecords {
		hrsCount += len(hrs)
	}
	m.metrics.K8gbGslbHealthyRecords.With(prometheus.Labels{"namespace": rs.NamespacedName.Namespace, "name": rs.NamespacedName.Name}).Set(float64(hrsCount))
}
...
func (m *PrometheusMetrics) UpdateFailoverStatus(rs *rs.ReconciliationState, isPrimary bool, healthy status.HealthStatus, targets []string) {
	t := secondary
	if isPrimary {
		t = primary
	}
...
func (m *PrometheusMetrics) UpdateRoundrobinStatus(rs *rs.ReconciliationState, healthy status.HealthStatus, targets []string) {
	m.updateRuntimeStatus(rs, m.metrics.K8gbGslbStatusCountForRoundrobin, healthy, targets, "")
}

func (m *PrometheusMetrics) UpdateGeoIPStatus(gslb *rs.ReconciliationState, healthy status.HealthStatus, targets []string) {
	m.updateRuntimeStatus(gslb, m.metrics.K8gbGslbStatusCountForGeoip, healthy, targets, "")
}

func (m *PrometheusMetrics) IncrementError(rs *rs.ReconciliationState) {
...
func (m *PrometheusMetrics) IncrementReconciliation(rs *rs.ReconciliationState) {
...
func (m *PrometheusMetrics) InfobloxIncrementZoneUpdate(rs *rs.ReconciliationState) {
...
func (m *PrometheusMetrics) InfobloxIncrementZoneUpdateError(rs *rs.ReconciliationState) {
...
func (m *PrometheusMetrics) InfobloxIncrementHeartbeat(rs *rs.ReconciliationState) {
...
func (m *PrometheusMetrics) InfobloxIncrementHeartbeatError(rs *rs.ReconciliationState) {
