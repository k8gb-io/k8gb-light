...
//
//var (
//	defaultGslb     = new(rs.ReconciliationState)
...
//	// arrange
//	m := newPrometheusMetrics(defaultConfig)
//	var serviceHealth = map[string]status.HealthStatus{
//		"roundrobin.cloud.example.com": status.Healthy,
//		"failover.cloud.example.com":   status.Healthy,
//		"unhealthy.cloud.example.com":  status.Unhealthy,
//		"notfound.cloud.example.com":   status.NotFound,
//	}
//	// act
//	cntHealthy1 := testutil.ToFloat64(m.Get(K8gbGslbServiceStatusNum).AsGaugeVec().With(
//		prometheus.Labels{"namespace": namespace, "name": gslbName, "status": status.Healthy.String()}))
//	cntUnhealthy1 := testutil.ToFloat64(m.Get(K8gbGslbServiceStatusNum).AsGaugeVec().
//		With(prometheus.Labels{"namespace": namespace, "name": gslbName, "status": status.Unhealthy.String()}))
//	cntNotFound1 := testutil.ToFloat64(m.Get(K8gbGslbServiceStatusNum).AsGaugeVec().
//		With(prometheus.Labels{"namespace": namespace, "name": gslbName, "status": status.NotFound.String()}))
...
//		With(prometheus.Labels{"namespace": namespace, "name": gslbName, "status": status.Healthy.String()}))
...
//
//	// act
//	m.UpdateFailoverStatus(defaultGslb, true, status.Healthy, []string{"10.0.0.1", "10.0.0.2"})
//	// assert
//	hp := testutil.ToFloat64(m.Get(K8gbGslbStatusCountForFailover).AsGaugeVec().
//		With(prometheus.Labels{"namespace": namespace, "name": gslbName, "status": fmt.Sprintf("%s_%s", status.Healthy, primary)}))
//	up := testutil.ToFloat64(m.Get(K8gbGslbStatusCountForFailover).AsGaugeVec().
//		With(prometheus.Labels{"namespace": namespace, "name": gslbName, "status": fmt.Sprintf("%s_%s", status.Unhealthy, primary)}))
//	fp := testutil.ToFloat64(m.Get(K8gbGslbStatusCountForFailover).AsGaugeVec().
//		With(prometheus.Labels{"namespace": namespace, "name": gslbName, "status": fmt.Sprintf("%s_%s", status.NotFound, primary)}))
//	assert.Equal(t, 2., hp)
//	assert.Equal(t, 0., up)
//	assert.Equal(t, 0., fp)
//
//	// act
//	m.UpdateFailoverStatus(defaultGslb, false, status.Unhealthy, []string{"10.0.1.1", "10.0.1.2"})
//	// assert
//	hs := testutil.ToFloat64(m.Get(K8gbGslbStatusCountForFailover).AsGaugeVec().
//		With(prometheus.Labels{"namespace": namespace, "name": gslbName, "status": fmt.Sprintf("%s_%s", status.Healthy, secondary)}))
//	us := testutil.ToFloat64(m.Get(K8gbGslbStatusCountForFailover).AsGaugeVec().
//		With(prometheus.Labels{"namespace": namespace, "name": gslbName, "status": fmt.Sprintf("%s_%s", status.Unhealthy, secondary)}))
//	fs := testutil.ToFloat64(m.Get(K8gbGslbStatusCountForFailover).AsGaugeVec().
//		With(prometheus.Labels{"namespace": namespace, "name": gslbName, "status": fmt.Sprintf("%s_%s", status.NotFound, secondary)}))
//	assert.Equal(t, 0., hs)
//	assert.Equal(t, 2., us)
//	assert.Equal(t, 0., fs)
//}
...
//
//	// act
//	m.UpdateRoundrobinStatus(defaultGslb, status.Healthy, []string{"10.0.0.1", "10.0.0.2"})
//	// assert
...
//
//	// act
//	m.UpdateRoundrobinStatus(defaultGslb, status.Unhealthy, []string{"10.0.1.1", "10.0.1.2"})
//	// assert
