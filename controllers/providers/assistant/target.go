...
func (t Targets) FailoverProjection() Targets {
	var tags []string
	tt := NewTargets()
	if len(t) == 0 {
		return tt
	}
	// if primary exists, return primary target only
	for k, v := range t {
		if v.IsPrimary {
			tt.Append(k, v.IPs, true)
			return tt
		}
		tags = append(tags, k)
	}
	// only secondary tags exists
	sort.Strings(tags)
	firstTag := tags[0]
	tt.Append(firstTag, t[firstTag].IPs, false)
	return tt
}
